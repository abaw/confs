...
import           XMonad.Hooks.ManageHelpers  (Side (..), doRectFloat, currentWs)
...
main = let config = mateConfig
                    ...
                    , logHook = updateLastWindow >> updateLastTerminal
                    , startupHook = startupHook mateConfig >> loadXmodmap >> checkKeymap config myKeyBindings >> dedicatedTerm
...
myManageHook = composeAll [ currentWs >>= doF . W.view
...
loadXmodmap = safeSpawn "xmodmap" ["~/.Xmodmap"]

skype = runOrRaiseNext "skype" $ (resource =? "skype")
...
emacs = runOrRaiseNext "emacs" (className =? "Emacs24" <||> className =? "Emacs" <||> className =? "emacs")
...
switchScreenConfiguration = runSelectedAction myGSConfig $
  [ ("Builtin+External(Primary)", safeSpawn "xrandr" ["--output",  "DP1", "--primary", "--auto", "--output", "eDP1", "--mode", "1360x768", "--right-of", "DP1" ])
  , ("Builtin(Primary)+External", safeSpawn "xrandr" ["--output",  "eDP1", "--primary", "--auto", "--output", "DP1", "--left-of", "eDP1", "--auto"])
